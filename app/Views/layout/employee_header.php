...
<?php
$userName = (string) Session::get('user_name', 'Employee');
$userInitials = strtoupper(mb_substr(trim($userName), 0, 1)) ?: 'E';
?>

<div class="ems-admin-shell">
    <aside class="ems-sidebar d-none d-md-flex flex-column">
        <div class="ems-sidebar-brand">
            <div class="ems-logo"><?= htmlspecialchars($userInitials, ENT_QUOTES, 'UTF-8') ?></div>
            <div class="ms-2">
        ...
        <ul class="nav flex-column px-2 gap-1">
            <li class="nav-item">
                <a class="nav-link ems-navlink <?= $isActive('/dashboard') ? 'active' : '' ?>" href="<?= $base ?>/dashboard">
                    <i class="bi bi-grid"></i><span>Dashboard</span>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link ems-navlink <?= $isActive('/profile') ? 'active' : '' ?>" href="<?= $base ?>/profile">
            ...
            <li class="nav-item">
                <a class="nav-link ems-navlink <?= $isActive('/leave') ? 'active' : '' ?>" href="<?= $base ?>/leave">
        ...
        <div class="mt-auto px-3 pb-3">
            <form method="post" action="<?= $base ?>/logout" class="mt-2">
    ...
    <div class="ems-main">
        <header class="ems-topbar border-bottom">
            <div class="d-flex align-items-center gap-2">
                <button class="btn btn-outline-secondary btn-sm d-md-none" type="button" data-bs-toggle="offcanvas" data-bs-target="#employeeSidebarOffcanvas" aria-controls="employeeSidebarOffcanvas">
                ...
                <?php if (Auth::check() && !Auth::isAdmin()): ?>
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            <li><a class="dropdown-item" href="<?= $base ?>/profile">My Profile</a></li>
                            <li><hr class="dropdown-divider"></li>
                            <li>
                                <form method="post" action="<?= $base ?>/logout" class="px-3 py-1 m-0">
                                    <input type="hidden" name="_csrf" value="<?= Csrf::token() ?>">
        ...
        <main class="container-fluid py-4">
            ...
            <div class="offcanvas offcanvas-start" tabindex="-1" id="employeeSidebarOffcanvas" aria-labelledby="employeeSidebarOffcanvasLabel">
                <div class="offcanvas-header">
                    <h5 class="offcanvas-title" id="employeeSidebarOffcanvasLabel">EMS</h5>
                ...
                <div class="offcanvas-body p-0">
                    <div class="ems-sidebar ems-sidebar-mobile">
                        ...
                        <ul class="nav flex-column px-2 gap-1 mt-2">
                            <li class="nav-item">
                                <a class="nav-link ems-navlink <?= $isActive('/dashboard') ? 'active' : '' ?>" href="<?= $base ?>/dashboard"><i class="bi bi-grid"></i><span>Dashboard</span></a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link ems-navlink <?= $isActive('/profile') ? 'active' : '' ?>" href="<?= $base ?>/profile"><i class="bi bi-person"></i><span>My Profile</span></a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link ems-navlink <?= $isActive('/leave') ? 'active' : '' ?>" href="<?= $base ?>/leave"><i class="bi bi-calendar2-week"></i><span>Leave</span></a>
                            </li>
                        </ul>
